...
def listar_tipos_menu(db: Session = Depends(get_db)):
    try:
        datos = get_all(db=db)
        return objRespuesta(respuesta=True, data=datos)
    except Exception as e:
        return objRespuesta(respuesta=False, mensaje=str(e), data=[])
...
def crear_tipo_menu(data: TipoMenuCreate, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Error al crear tipo de menú: {str(e)}")
