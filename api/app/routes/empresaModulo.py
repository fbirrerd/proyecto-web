...
def crear_empresa_modulo(obj: EmpresaModuloCreate, db: Session = Depends(get_db)):
    try:
        data = create_empresa_modulo(db, obj)
...
def read_empresas_modulo(db: Session = Depends(get_db)):
    try:
        data = get_empresa_modulo(db)
        return objRespuesta(
            respuesta = True, 
            data = data
        )
...
def guardar_relacion(obj: EmpresaModuloRelacion, db: Session = Depends(get_db)):
    try:
        data = create_relacion_empresa_modulo(db, obj)
        return objRespuesta(respuesta = True, data = data)
    except Exception as e:
